    ...
    <div className={`bv-mm-panel ${active ? 'open' : ''}`} data-pane={active || ''}>
      ...
      {active === 'company' && (
        <div className="bv-mm-pane active">
          ...
          <div className="bv-mm-grid">
            <CompanyCard href="/who-we-are" t={t('mm.about.who')} s={t('mm.about.who.s')} icon={<UserIcon />} bg="linear-gradient(135deg,#BFEFEA,#4FC2B6)" onClose={onClose} />
            <CompanyCard href="/projects" t={t('mm.about.proj')} s={t('mm.about.proj.s')} icon={<PhotoIcon />} bg="linear-gradient(135deg,#FFD9B8,#F2945A)" onClose={onClose} />
            <CompanyCard href="/news" t={t('mm.about.news')} s={t('mm.about.news.s')} icon={<NewsIcon />} bg="linear-gradient(135deg,#CBD6F7,#6E84E0)" onClose={onClose} />
          </div>
        </div>
      )}
...
function CompanyCard({ href, t, s, icon, bg, onClose }: { href: string; t: string; s: string; icon: any; bg: string; onClose: () => void }) {
  return (
    <Link className="bv-mm-card" href={href} onClick={onClose}>
      <div className="bv-mm-ico" style={{ background: bg }}>{icon}</div>
...
const UserIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4.4 3.6-8 8-8s8 3.6 8 8"/></svg>
const PhotoIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10l6 4 4-3 8 5"/></svg>
const NewsIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M7 8h10M7 12h10M7 16h6"/></svg>
const QuestionIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><circle cx="12" cy="12" r="9"/><path d="M9.5 9a2.5 2.5 0 0 1 5 0c0 1.5-2.5 2.5-2.5 4M12 17h.01"/></svg>
const ChatIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
const CheckIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><path d="M9 12l2 2 4-4M5 5h14v14H5z"/></svg>
const LockIcon = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1a1f3a" strokeWidth="1.7"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
